...
MAX_CF_RETRIES = max(1, int(os.environ.get("OUTLOOK_MAX_CF_RETRIES", "3")))
...
def run_one(
    ...
    if not result.get("success"):
        ...
        # v10.1: "CAPTCHA已点击但页面未跳转" = MS后端拒绝了session，不是CF IP问题
        #   → 不ban CF IP，只重试2次（不是9次），节省pool IPs
        _is_ms_reject = "页面未跳转" in err
        if ip_info and any(kw in err for kw in (
            "验证码", "CAPTCHA", "IP质量", "频率过快", "ERR_TUNNEL", "ERR_CONNECTION",
            "Timeout", "等待同意按钮", "Consent", "net::ERR",
        )):
            if not _is_ms_reject:
                _ban_cf_ip(cur_exit)
            # patch7/A: 记录Arkose子网失败统计（仅真实CAPTCHA失败，不含MS后端拒绝）
            if cur_exit and any(kw in err for kw in ("验证码", "CAPTCHA")) and not _is_ms_reject:
                try:
            ...
            # v10.1: MS后端拒绝session → 最多重试2次
            if _is_ms_reject:
                result["_ms_reject_count"] = result.get("_ms_reject_count", 0) + 1
        return result
...
def main():
    ...
    for i in range(args.count):
        ...
        # CF IP 重试循环
        r = None
        _retry_email    = pe   # 换IP重试时复用已生成邮箱，省表单重填
        _retry_password = pp
        _ms_reject_total = 0  # v10.1: 跨轮累计MS后端拒绝次数（每次run_one都是新dict）
        for cf_attempt in range(MAX_CF_RETRIES):
            r = run_one(
                idx=i, total=args.count,
                planned_email=_retry_email,
                planned_password=_retry_password,
            ...
            if r.get("success"):
                ...
                try:
                    ...
                    pass
                break
            # 捕获本次已生成的邮箱/密码，供下次CF IP重试复用（避免重填表单）
            if not _retry_email and r.get("email"):
                _retry_email    = r["email"]
                _retry_password = r.get("password", "")
            if not r.get("_should_retry"):
                break
            # v10.1: MS后端拒绝（页面未跳转）在外层循环累计，最多2次后放弃此邮箱换新邮箱
            if "页面未跳转" in r.get("error", ""):
                _ms_reject_total += 1
                if _ms_reject_total >= 2:
                    log(f"  ⚠ MS后端连续拒绝{_ms_reject_total}次 → 放弃此邮箱，换新邮箱")
                    break
            log(f"  ↺ CF IP 重试 ({cf_attempt+1}/{MAX_CF_RETRIES})，复用邮箱={_retry_email or '未生成'}...")
            time.sleep(3)
