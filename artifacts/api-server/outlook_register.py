...
OUTLOOK_SCREENSHOT_DEBUG = os.environ.get("OUTLOOK_SCREENSHOT_DEBUG", "0").lower() in ("1", "true", "yes", "on")
OUTLOOK_SCREENSHOT_FAILURES = os.environ.get("OUTLOOK_SCREENSHOT_FAILURES", "0").lower() in ("1", "true", "yes", "on")

def _maybe_screenshot(page, path: str, *, failure: bool = False, force: bool = False) -> bool:
    if not (force or OUTLOOK_SCREENSHOT_DEBUG or (failure and OUTLOOK_SCREENSHOT_FAILURES)):
        return False
    try:
...
class BaseController:
    ...
    def outlook_register(self, page, email, password):
        ...
        try:
            ...
            # 截图记录提交用户名后的页面（方便调试）
            try:
                _maybe_screenshot(page, f"/tmp/outlook_after_username_{email}.png")
            except Exception:
                pass
            ...
            if (page.get_by_text(TXT_UNUSUAL_ACTIVITY).count()
                ...
                try: _maybe_screenshot(page, f"/tmp/ratelimit_A_{email}.png", failure=True)
            ...
            if page.get_by_text(TXT_TRANSIENT_ERROR).count():
                try: _maybe_screenshot(page, f"/tmp/transient_A_{email}.png", failure=True)
                except Exception: pass
            ...
            _cap_retry_count = 0
            for _cc_i in range(30):  # v11.2: 30x2s = 60s (原23=46s，多轮Arkose挑战需要更长)
                try:
                    ...
                    if "signup.live.com" not in _cur_url:
                        ...
                        break
                    # v11.3: early_confirmed = px-captcha 经 3s 核查确认已清空（CAPTCHA 真正通过）
                    # body text 由微软 SPA 延迟更新，不可靠；10s 后直接信任 early_confirmed
                    if getattr(self, "_captcha_early_confirmed", False) and _cc_i >= 5:
                        _cap_text_cleared = True
                        print(f"[register] [captcha-clear] early_confirmed 10s后直接通过 ({_cc_i*2}s)", flush=True)
                        break
                    try:
                    ...
                    _ec = getattr(self, "_captcha_early_confirmed", False)
                    # v11.2+fix: early_confirmed=False -> up to 7 retries (original v11.2)
                    # early_confirmed=True  -> ONE safety retry at 26s only
                    if ((_cc_i in (5, 9, 13, 17, 21, 25, 29) and not _ec and _cap_retry_count < 7)
                            or (_cc_i == 13 and _ec and _cap_retry_count == 0)):
                        _cap_retry_count += 1
                        print(f"[register] [captcha-clear] v11.2+fix 重试#{_cap_retry_count} at {_cc_i*2}s (early_confirmed={_ec})", flush=True)
                        try:
                            ...
                            if "signup.live.com" not in _ur2:
                                ...
                                break
                            # v11.3: early_confirmed 路径安全重试已通过 → 直接认为清空，不继续空转
                            if _ec and _retry_ok:
                                _cap_text_cleared = True
                                print(f"[register] [captcha-clear] early_confirmed 安全重试OK → 直接通过", flush=True)
                                break
            ...
            if not _cap_text_cleared:
                try:
                    _maybe_screenshot(page, f"/tmp/outlook_captcha_stuck_{email}.png", failure=True)
                except Exception:
                    pass
            ...
            try:
                ...
                if _phone_page:
                    # v9.80 bugfix: /proofs/Add 同时列出邮件和手机选项，但页面可跳过
                    # 先尝试点 "Skip for now"，成功则继续注册
                    _proofs_skipped = False
                    try:
                        for _sk_sel in [
                            'a:has-text("Skip for now")', 'button:has-text("Skip for now")',
                            'a:has-text("skip for now")', 'a:has-text("暂时跳过")',
                            'button:has-text("暂时跳过")', 'a:has-text("跳过")',
                            'a[id*="idA_SAOTCS_Skip" i]', 'button[id*="skip" i]',
                        ]:
                            try:
                                _skloc = page.locator(_sk_sel).first
                                if _skloc.is_visible(timeout=1200):
                                    _skloc.click()
                                    page.wait_for_timeout(2000)
                                    print(f"[register] ✅ v9.80 /proofs 页已跳过 ({_sk_sel})，继续注册", flush=True)
                                    _proofs_skipped = True
                                    break
                            except Exception:
                                continue
                    except Exception:
                        pass
                    if not _proofs_skipped:
                        # 无法跳过 → 确认是否真的有手机输入框（排除文本误判）
                        _has_tel_input = page.locator('input[type="tel"], input[name="PhoneNumber"]').count() > 0
                        if _has_tel_input:
                            try:
                                _maybe_screenshot(page, f"/tmp/outlook_phone_page_{email}.png", failure=True)
                            except Exception:
                                pass
                            print(f"[register] ⚠ 注册后出现强制手机验证页（无法跳过），放弃此次注册", flush=True)
                            return False, "phone_verification_required", email
                        else:
                            print(f"[register] [v9.80] phone_page 文本误判（无 tel input，无跳过按钮），继续尝试", flush=True)
                    ...
                    if _pp_inp or _pp_txt:
                        ...
                        try:
                            for _sk2_sel in [
                                ...
                                try:
                                    _sk2loc = page.locator(_sk2_sel).first
                                    if _sk2loc.is_visible(timeout=1200):
                                        _sk2loc.click()
                                        page.wait_for_timeout(2000)
                                        print(f"[register] ✅ v9.80 inline phone页已跳过 ({_sk2_sel})，继续注册", flush=True)
                                        _pp_skipped = True
                                        break
                                except Exception:
                                    continue
                            ...
                            pass
                        if not _pp_skipped:
                            _has_tel2 = page.locator('input[type="tel"], input[name="PhoneNumber"]').count() > 0
                            if _has_tel2:
                                try:
                                    page.screenshot(path=f"/tmp/outlook_phone_page_{email}.png")
                                except Exception:
                                    pass
                                print(f"[register] ⚠ 注册后出现强制手机验证页(inline，无法跳过), url={page.url[:80]}", flush=True)
                                return False, "phone_verification_required", email
                            else:
                                print(f"[register] [v9.80] inline phone_txt 误判（无 tel input），继续", flush=True)
            ...
            ok_signal = False
            # ── 第零层: passkey/卡页预检 — cookie已存在直接跳过60s wait_for_url ──────
            if _has_success_cookie():
                print(f"[register] ✅ pre-wait cookie预检命中，跳过wait_for_url (passkey等情形节省~60s) url={page.url[:80]}", flush=True)
                ok_signal = True
            if not ok_signal:
              try:
            ...
            if not ok_signal:
                cur_url = page.url
                try:
                    _maybe_screenshot(page, f"/tmp/outlook_captcha_done_{email}.png", failure=True)
                except Exception:
                    pass
                _body_snip = ""
                try:
...
class PatchrightController(BaseController):
    ...
    @staticmethod
    def _human_press_hold(page, cx, cy, hold_ms_min=2800, hold_ms_max=3300):  # v11.2: ~3s
        ...
        page.mouse.down()
        # v11.2 fix: 30-60ms 高频微动，模拟真实人手连续微颗（原 180-360ms 太稀疏，Arkose检测为冻结-跳跃机器人）
        _hold_total = _r.randint(hold_ms_min, hold_ms_max)
        _elapsed = 0
        while _elapsed < _hold_total:
            _wait = _r.randint(30, 60)  # v11.2: 30-60ms/次 vs 原 180-360ms
            page.wait_for_timeout(_wait)
            _elapsed += _wait
            _jx = cx + _r.uniform(-0.4, 0.4)  # v11.2: 微颗 ±0.4px vs 原 ±2.2px
            _jy = cy + _r.uniform(-0.4, 0.4)
            try:
        ...
        for attempt in range(self.max_retries + 1):
            ...
            if _second_click_done:
                ...
                try:
                    ...
                    if _box2 and _box2['width'] > 0:
                        ...
                        _held = PatchrightController._human_press_hold(page, _cx2, _cy2, 2800, 3300)  # v11.2: Arkose expects ~3s
                ...
                if not _press_clicked:
                    ...
                    for _jfr in _js_frames:
                        try:
                            ...
                            """)
                            if _js_r:
                                print(f"[captcha] ✅ 方法A-JS: frame press-hold 3.0s (btn={_js_r})", flush=True)
                                _press_clicked = True
                                    ...
                                    for _rhfr in page.frames:
                                        try:
                                            ...
                                            if _px_box and _px_box.get('width', 0) > 0:
                                                ...
                                                _held_px = PatchrightController._human_press_hold(page, _rx, _ry, 3000, 3500)  # v11.2: Arkose expects ~3s
            ...
            try:
                ...
                if _early_solved_reason:
                    # v11.2: 3s 确认等待 — px-captcha 在 Arkose 多轮挑战时会过渡性短暂清空
                    # 等3s后再核查，仍为空=真正通过；否则是新一轮挑战加载中=假阳性
                    try:
                        page.wait_for_timeout(3000)
                        _confirm_still_empty = False
                        for _pf_recheck in page.frames:
                            _u_rc = getattr(_pf_recheck, "url", "") or ""
                            if "hsprotect.net" not in _u_rc:
                                continue
                            _px_rc = _pf_recheck.evaluate("""() => {
                                const root = document.getElementById("px-captcha");
                                if (!root || root.children.length === 0) return "empty";
                                const inner = root.querySelector("iframe");
                                if (inner) {
                                    const cs = window.getComputedStyle(inner);
                                    if (cs.display==="none"||cs.visibility==="hidden") return "empty";
                                }
                                return "reloaded";
                            }""")
                            if _px_rc == "empty":
                                _confirm_still_empty = True
                            break  # 只检查第一个 hsprotect frame
                    except Exception:
                        _confirm_still_empty = True  # 确认异常=保守认为通过
                    if not _confirm_still_empty:
                        print(f"[captcha] ⚠ v11.2: px-captcha 3s后已重新加载（多轮Arkose挑战），继续按压…", flush=True)
                        # 不 return True，继续外层 attempt 循环进行下一次按压
                    else:
                        print(f"[captcha] ✅ press-hold 后早期检测(3s已确认)：CAPTCHA 已通过（{_early_solved_reason}）→ 跳过音频流程", flush=True)
            ...
            try:
                if _maybe_screenshot(page, f"/tmp/outlook_captcha_after_a11y_{attempt}.png"):
...
def get_oauth_token_in_browser(page, email: str, captcha_handler=None, password: str = "") -> dict:
    ...
    try:
        ...
        for _poll_round in range(5):
            ...
            try:
                if OUTLOOK_SCREENSHOT_DEBUG:
                    _shot_p = f"/tmp/oauth_round_{email}_{_poll_round+1}.png"
                    _maybe_screenshot(page, _shot_p, force=True)
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # 截图留档（住宅代理轮）
                            try:
                                _maybe_screenshot(_res_page, f'/tmp/oauth_round_{email}_res.png')
                            except Exception:
                                pass
                            # 尝试点击 Accept/同意 按鈕
...
def register_one(ctrl, engine_name: str, headless: bool, planned_username: str = "", planned_password: str = "", exit_ip: str = "", proxy_port: int = 0, proxy_formatted: str = "") -> dict:
    ...
    try:
        ...
        if ok:
            ...
            try:
                _maybe_screenshot(page, f"/tmp/outlook_ok_{actual_email}.png")
            except Exception:
                pass
        else:
            try:
                _maybe_screenshot(page, f"/tmp/outlook_fail_{actual_email}.png", failure=True)
            except Exception:
                pass
        ...
        result["error"] = str(e)
        try:
            _maybe_screenshot(page, f"/tmp/outlook_err_{email}.png", failure=True)
        except Exception:
            pass
    finally:
...
def run_parallel(args) -> None:
    ...
    def worker_thread(thread_id):
        ...
        while True:
            ...
            t0 = _tm.time()
            _kpf_ip_retries = 0
            _KPF_MAX_IP_RETRIES = 2  # Fix-Bug1: retry on ERR_CONNECTION with fresh CF IP
            while True:
                ...
                # Fix-Bug1: kpf网络错误换IP重试（ERR_CONNECTION/ERR_TUNNEL/等待同意按钮超时）
                if (use_cf and _cf_par is not None and results
                        and not results[0].get("success")
                        and _kpf_ip_retries < _KPF_MAX_IP_RETRIES):
                    _kerr = results[0].get("error", "")
                    if any(k in _kerr for k in ("ERR_CONNECTION", "net::ERR", "等待同意按钮", "ERR_TUNNEL")):
                        _kpf_ip_retries += 1
                        with lock:
                            print("[T%02d] ⚠ kpf换IP重试 %d/%d: %s" % (
                                task_idx+1, _kpf_ip_retries, _KPF_MAX_IP_RETRIES, _kerr[:80]), flush=True)
                        continue
                break
