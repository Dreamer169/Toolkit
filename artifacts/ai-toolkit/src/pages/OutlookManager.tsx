...
export default function OutlookManager() {
  ...
  // 并发 workers
  const [regWorkers,  setRegWorkers]  = useState(1);
  ...
  const [schedStatus,  setSchedStatus]  = useState<SchedStatus | null>(null);
  const [schedEnabled, setSchedEnabled] = useState(false);
  const [schedTarget,  setSchedTarget]  = useState(500);
  const [schedDaily,   setSchedDaily]   = useState(24);
  const [schedFloat,   setSchedFloat]   = useState(0.2);
  const [schedEngine,  setSchedEngine]  = useState("patchright");
  const [schedPMode,   setSchedPMode]   = useState("cf");
  const [schedBusy,    setSchedBusy]    = useState(false);
  const [schedOpen,    setSchedOpen]    = useState(false);
  const schedPollRef = useRef<ReturnType<typeof setInterval>|null>(null);

  const fetchSchedStatus = async () => {
    try {
      const r = await fetch("/api/tools/outlook/schedule");
      const d = await r.json() as { success: boolean } & SchedStatus;
      if (d.success) {
        setSchedStatus(d);
        setSchedEnabled(d.enabled);
        setSchedTarget(d.targetCount);
        setSchedDaily(d.dailyCount);
        setSchedFloat(d.floatRatio);
        setSchedEngine(d.engine);
        setSchedPMode(d.proxyMode);
      }
    } catch {}
  };

  // 热更新：直接 POST，不需要先停止再启动
  const applySchedule = async (enabled: boolean) => {
    setSchedBusy(true);
    try {
      const r = await fetch("/api/tools/outlook/schedule", {
        method: "POST",
        ...
        body: JSON.stringify({
          enabled,
          targetCount: schedTarget,
          dailyCount:  schedDaily,
          floatRatio:  schedFloat,
          engine:      schedEngine,
          proxyMode:   schedPMode,
        }),
      });
      const d = await r.json() as { success: boolean } & SchedStatus;
      if (d.success) { setSchedStatus(d); setSchedEnabled(d.enabled); }
    } catch {}
    setSchedBusy(false);
  };

  // 热更新参数（不改变 enabled 状态）
  const updateSchedConfig = async () => {
  ...
  useEffect(() => {
    if (schedOpen) {
      fetchSchedStatus();
      schedPollRef.current = setInterval(fetchSchedStatus, 10000);
    } else {
  ...
  const fmtNextRun = (ts: number | null) => {
    if (!ts) return "—";
  ...
  const startDeviceCodeFlow = async () => {
    ...
    try {
      ...
      // 固定过期时间戳，避免闭包内重复计算 Date.now() 导致条件永远为 false
      const savedExpiry = Date.now() + (d.expiresIn ?? 900) * 1000;
      dcPollRef.current = setInterval(async () => {
        if (Date.now() > savedExpiry) { stopDcPoll(); setDcStatus("error"); setDcErr("授权码已过期"); return; }
        try {
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#161b22] border border-[#21262d] rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              {/* 并发数 */}
              <div>
                <label className="text-[11px] text-gray-500 mb-1 block">并发 workers (推荐≤4)</label>
                <input type="number" min={1} max={16} value={regWorkers} onChange={e => setRegWorkers(+e.target.value)} disabled={regBusy}
                  className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg px-2 py-1.5 text-xs text-gray-300 focus:outline-none focus:border-blue-500 disabled:opacity-50" />
              </div>
            </div>
          ...
          <div className="bg-[#161b22] border border-[#21262d] rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 cursor-pointer select-none hover:bg-[#1c2129]"
              ...
              <div className="flex items-center gap-2">
                <span>⏱️</span>
                <span className="text-sm font-semibold text-white">定时自动注册</span>
                {schedStatus?.enabled && (
                  <span className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />运行中
                  </span>
                )}
                <svg className={`w-3.5 h-3.5 text-gray-600 transition-transform ml-0.5 ${schedOpen ? "rotate-180" : ""}`}
                  fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
                </svg>
              </div>
              <span className="text-[10px] text-gray-600">账号池低于目标时自动补充</span>
            </div>
            {schedOpen && (
              <div className="px-4 pb-4 space-y-4 border-t border-[#21262d]">
                ...
                {schedStatus && (
                  <div className="grid grid-cols-4 gap-2 pt-3">
                    {[
                      { label: "当前账号", value: schedStatus.currentCount, sub: `目标 ${schedStatus.targetCount}`,
                        tone: schedStatus.currentCount >= schedStatus.targetCount ? "ok" : "warn" },
                      { label: "已触发", value: schedStatus.runCount, sub: "次"},
                      { label: "下次运行", value: fmtNextRun(schedStatus.nextRunAt), sub: fmtTime(schedStatus.nextRunAt) },
                      { label: "上次运行", value: schedStatus.lastRunResult
                          ? `${schedStatus.lastRunResult.registered}/${schedStatus.lastRunResult.attempted}`
                          : "—",
                        sub: fmtTime(schedStatus.lastRunAt),
                        tone: schedStatus.lastRunResult?.error ? "err" : "dim" },
                    ].map((s, i) => {
                      const color = s.tone === "ok" ? "text-emerald-400" : s.tone === "warn" ? "text-amber-400"
                        : s.tone === "err" ? "text-red-400" : "text-white";
                      return (
                        <div key={i} className="bg-[#0d1117] border border-[#21262d] rounded-lg px-3 py-2">
                          <div className="text-[10px] text-gray-500 uppercase tracking-wide truncate">{s.label}</div>
                          <div className={`text-sm font-bold ${color} leading-tight`}>{s.value}</div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  <div>
                    <label className="text-[11px] text-gray-500 mb-1 block">目标账号数上限</label>
                    <input type="number" min={1} max={9999} value={schedTarget}
                      onChange={e => setSchedTarget(+e.target.value)}
                      className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg px-2 py-1.5 text-xs text-gray-300 focus:outline-none focus:border-blue-500" />
                  </div>
                  <div>
                    <label className="text-[11px] text-gray-500 mb-1 block">每日注册数</label>
                    <input type="number" min={1} max={288} value={schedDaily}
                      onChange={e => setSchedDaily(+e.target.value)}
                      className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg px-2 py-1.5 text-xs text-gray-300 focus:outline-none focus:border-blue-500" />
                  </div>
                  <div>
                    <label className="text-[11px] text-gray-500 mb-1 block">浮动比例 (±%)</label>
                    <input type="number" min={0} max={50} step={5}
                      value={Math.round(schedFloat * 100)}
                      onChange={e => setSchedFloat(+e.target.value / 100)}
                      className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg px-2 py-1.5 text-xs text-gray-300 focus:outline-none focus:border-blue-500" />
                  </div>
                  <div>
                    <label className="text-[11px] text-gray-500 mb-1 block">引擎</label>
                    <select value={schedEngine} onChange={e => setSchedEngine(e.target.value)}
                      className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg px-2 py-1.5 text-xs text-gray-300 focus:outline-none focus:border-blue-500">
                  ...
                  <div>
                    <label className="text-[11px] text-gray-500 mb-1 block">代理模式</label>
                    <select value={schedPMode} onChange={e => setSchedPMode(e.target.value)}
                      className="w-full bg-[#0d1117] border border-[#30363d] rounded-lg px-2 py-1.5 text-xs text-gray-300 focus:outline-none focus:border-blue-500">
                      <option value="cf">CF IP 代理池</option>
                      <option value="">无代理</option>
                    </select>
                  </div>
                  {schedEnabled && (
                    <div className="flex items-end">
                      <button onClick={updateSchedConfig} disabled={schedBusy}
                        className="w-full py-1.5 bg-blue-600/60 hover:bg-blue-600 disabled:opacity-50 rounded-lg text-white text-xs font-medium transition-all">
                        {schedBusy ? "更新中…" : "↻ 热更新计划"}
                      </button>
                    </div>
                  )}
                ...
                <div className="flex gap-2">
                  {!schedEnabled ? (
                    <button onClick={() => applySchedule(true)} disabled={schedBusy}
                      className="flex-1 py-2 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 rounded-lg text-white text-sm font-medium transition-all">
                      {schedBusy ? "处理中..." : "▶ 启动定时注册"}
                    </button>
                  ) : (
                    <button onClick={() => applySchedule(false)} disabled={schedBusy}
                      className="flex-1 py-2 bg-red-600/80 hover:bg-red-700 disabled:opacity-50 rounded-lg text-white text-sm font-medium transition-all">
                      {schedBusy ? "处理中..." : "■ 停止定时注册"}
                    </button>
                  )}
                  <button onClick={runSchedNow} disabled={schedBusy}
                    className="px-4 py-2 bg-blue-600/70 hover:bg-blue-700 disabled:opacity-50 rounded-lg text-white text-sm font-medium transition-all">
                    立即运行
                  </button>
                </div>
                {schedStatus?.lastRunResult?.error && (
                  <p className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded px-2 py-1">
                    ⚠ 上次错误: {schedStatus.lastRunResult.error}
                  </p>
                )}
              </div>
            )}
