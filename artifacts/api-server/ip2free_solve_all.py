#!/usr/bin/env python3
"""
ip2free solve_all v3 - press_sequentially login + JS fill captcha
"""
...
SS_DIR = "/tmp/ip2free_solve_all_ss"
os.makedirs(SS_DIR, exist_ok=True)
LOG_F = open("/tmp/ip2free_solve_all.log", "a", buffering=1)
...
def solve_account(email, pw, port):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # press_sequentially bypasses readonly restriction
...
def main():
    log("\n" + "=" * 60)
    log("ip2free solve_all v3 | " + datetime.datetime.now().strftime("%H:%M:%S"))
    log("=" * 60)
    ...
    for round_n in range(1, MAX_ROUNDS + 1):
        if len(DONE) >= 4:
            break
        log("\n" + "=" * 50)
        log("ROUND " + str(round_n) + " | done=" + str(len(DONE)) + "/4")
        log("=" * 50)
        ...
        for email, pw, port in ACCOUNTS:
            ...
            try:
                ...
                log("pre-check err " + email + ": " + str(e)[:50])
                continue

            log(email + ": captcha=" + str(sz) + "b")
            if sz <= 100:
                log("  blocked, skip")
                continue
        ...
        log("Pending: " + str(pending))

        if pending and len(DONE) < 4:
            log("sleeping 240s...")
            time.sleep(240)
    ...
    with open("/tmp/ip2free_solve_all_result.json", "w") as f:
