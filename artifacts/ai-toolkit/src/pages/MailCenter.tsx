...
export default function MailCenter() {
  ...
  // recv-test state
  const [recvTestBusy, setRecvTestBusy] = useState(false);
  const [recvTestResult, setRecvTestResult] = useState<{ ok: boolean; msg: string; addr?: string } | null>(null);
  const recvTestPollRef = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  const runRecvTest = async () => {
    if (!selAccount) return;
    if (recvTestPollRef.current) clearInterval(recvTestPollRef.current);
    setRecvTestBusy(true);
    setRecvTestResult(null);
    try {
      const allocRes = await fetch(`${API}/tools/mailhub/alloc`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ service: 'mailcenter-recv-test' }),
      }).then(r => r.json()).catch(() => ({ success: false, error: '网络错误' })) as { success: boolean; id?: string; address?: string; error?: string };
      if (!allocRes.success || !allocRes.id || !allocRes.address) {
        setRecvTestResult({ ok: false, msg: '❌ 分配临时收件筱失败：' + (allocRes.error ?? '未知错误') });
        setRecvTestBusy(false);
        return;
      }
      const inboxId = allocRes.id;
      const inboxAddr = allocRes.address;
      setRecvTestResult({ ok: false, msg: `⏳ 已分配 ${inboxAddr}，正在发送…`, addr: inboxAddr });

      const sendRes = await fetch(`${API}/tools/outlook/send-message`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          accountId: selAccount.id,
          to: inboxAddr,
          subject: `收件测试 ${new Date().toLocaleTimeString('zh-CN')}`,
          body: `from ${selAccount.email} via Graph API`,
          bodyType: 'Text',
        }),
      }).then(r => r.json()).catch(() => ({ success: false, error: '网络错误' })) as { success: boolean; error?: string };
      if (!sendRes.success) {
        setRecvTestResult({ ok: false, msg: '❌ 发件失败：' + (sendRes.error ?? ''), addr: inboxAddr });
        setRecvTestBusy(false);
        return;
      }
      setRecvTestResult({ ok: false, msg: `✉ 已发出，等待 ${inboxAddr} 收件…`, addr: inboxAddr });

      let attempts = 0;
      recvTestPollRef.current = setInterval(async () => {
        attempts++;
        try {
          const poll = await fetch(`${API}/tools/mailhub/inbox/${inboxId}/messages`).then(r => r.json()).catch(() => null) as { messages?: unknown[] } | null;
          const msgs = poll?.messages ?? [];
          if (msgs.length > 0) {
            clearInterval(recvTestPollRef.current!); recvTestPollRef.current = null;
            setRecvTestResult({ ok: true, msg: `✅ 收件成功！${inboxAddr} 收到 ${msgs.length} 封邮件（${attempts * 3}s）`, addr: inboxAddr });
            setRecvTestBusy(false);
            fetch(`${API}/tools/mailhub/inbox/${inboxId}`, { method: 'DELETE' }).catch(() => {});
          } else if (attempts >= 20) {
            clearInterval(recvTestPollRef.current!); recvTestPollRef.current = null;
            setRecvTestResult({ ok: false, msg: `⚠ 超时（60s）未收到邮件，收件筱：${inboxAddr}`, addr: inboxAddr });
            setRecvTestBusy(false);
          }
        } catch {}
      }, 3000);
    } catch (e) {
      setRecvTestResult({ ok: false, msg: '❌ ' + String(e) });
      setRecvTestBusy(false);
    }
  };
  ...
  const checkIdleStatus = async () => {
    try {
      ...
      if (d?.success) {
        const hasPolling = d.accounts && Object.values(d.accounts as Record<string, {status:string}>).some(a => a.status === 'polling');
        setIdleDaemonRunning(!!d.running || !!hasPolling);
      }
    } catch { /* ignore */ }
  };
      ...
      {purgeStats && (
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              title="启动/停止 Graph 邮件轮询，实时捕获账号新邮件"
            >
              {idleDaemonBusy ? "…" : idleDaemonRunning ? "📡 Graph 监听中" : "📡 Graph 监听"}
            </button>
            {idleEvents.length > 0 && (
        ...
        {showCompose && selAccount ? (
          <div className="flex-1 flex flex-col p-5 gap-3 overflow-y-auto">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                onClick={runRecvTest}
                disabled={recvTestBusy}
                title="分配临时收件筱，用当前账号发测试邮件，自动验证收件"
                className="px-4 py-2 bg-emerald-700 hover:bg-emerald-600 disabled:opacity-50 rounded text-xs text-white font-semibold transition-colors"
              >
                {recvTestBusy ? "测试中…" : "🧪 收件测试"}
              </button>
              <button
                onClick={() => { setShowCompose(false); setSendResult(null); setRecvTestResult(null); }}
            ...
            {recvTestResult && (
              <div className={recvTestResult.ok
                ? "text-[11px] rounded px-3 py-2 border text-emerald-300 bg-emerald-950/40 border-emerald-800/40"
                : "text-[11px] rounded px-3 py-2 border text-amber-300 bg-amber-950/30 border-amber-800/30"}>
                {recvTestResult.msg}
              </div>
            )}
          </div>
