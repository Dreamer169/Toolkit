# pydoll browser/tab.py — CDP v7 snapshot (bypass_v7_applied)
# Restored with: scripts/restore_pydoll_cdp_v7.sh
# Contains: OOPIF-aware Method1 + deep shadow root Method2 + 25s token poll
# Applied on VPS: /usr/local/lib/python3.10/dist-packages/pydoll/browser/tab.py
        ...
        try:
            ...
            try:
                ...
                if _cf_target:
                    ...
                    # v4: strict selectors only; detect invisible Turnstile
                    await _aio.sleep(1.0)  # v6: reduced from 5.0
                    _click_remaining = max(12.0, time_to_wait_captcha - 5.0)
                    _click_deadline = _loop.time() + _click_remaining
                    _jscode = (
                        "function dc(root,sels){"
                        "for(var si=0;si<sels.length;si++){"
                        "var el=root.querySelector(sels[si]);"
                        "if(el){"
                        "var r=el.getBoundingClientRect();"
                        "if(r.width>0&&r.height>0){"
                        "try{el.click();}catch(e){}"
                        "try{el.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,view:window}));}catch(e){}"
                        "return 'clicked:'+sels[si]+':'+Math.round(r.width)+'x'+Math.round(r.height);"                        "}}}"
                        "var all=root.querySelectorAll('*');"
                        "for(var i=0;i<all.length;i++){if(all[i].shadowRoot){var r2=dc(all[i].shadowRoot,sels);if(r2)return r2;}}"
                        "return null;}"
                        "var sels=['span.cb-i','input[type=checkbox]','span[class=cb-i]','label[class*=cb-lb]'];"
                        "var res=dc(document,sels);"
                        "if(res)return res;"
                        "var anyBody=document.body?document.body.innerHTML.length:0;"
                        "return anyBody>50?'invisible':'wait';"
                    )
                    _got_invisible = False
                    while _loop.time() < _click_deadline:
                        try:
                            _res = await _cf_tab.execute_script(_jscode, return_by_value=True)
                            _r = _res if isinstance(_res, dict) else {}
                            _inner2 = _r.get('result', _r)
                            _val = str(_inner2.get('value', '')) if isinstance(_inner2, dict) else ''
                            if _val and _val.startswith('clicked'):
                                ...
                                return
                            if _val == 'invisible' and not _got_invisible:
                                logger.warning('[bypass] oopif v4: invisible Turnstile detected, waiting for auto-token')
                                _got_invisible = True
                                # Dump DOM for diagnosis
                                try:
                                    _dump = await _cf_tab.execute_script("document.body?document.body.innerHTML.substring(0,500):'empty'", return_by_value=True)
                                    _dr = _dump if isinstance(_dump, dict) else {}
                                    _di = _dr.get('result', _dr)
                                    _dv = str(_di.get('value', '')) if isinstance(_di, dict) else ''
                                    logger.warning(f'[bypass] oopif dom dump: {_dv[:300]}')
                                except Exception as _de:
                                    logger.warning(f'[bypass] oopif dom dump err: {_de}')
                        except Exception as _ejs:
                            logger.debug(f'[bypass] oopif exec err: {_ejs}')
                        await _aio.sleep(0.8)
                    if _got_invisible:
                        logger.debug('[bypass] oopif v4: invisible mode — returning to let caller wait for token')
                        return
                ...
                try:
                    ...
                    for _dsr in _deep_roots:
                        try:
                            ...
                            logger.warning('[bypass] deep-sr: clicked span.cb-i OK — polling 25s for token')
                            _m2_clicked = True
                            # bypass_v7_applied
