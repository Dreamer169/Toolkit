...
import Ip2freeProxy from "@/pages/Ip2freeProxy";
  ...
  const MORE_TABS: { id: Tab; label: string; icon: string; group: string }[] = [
    ...
    { id: "turb-register", label: "GPT注册", icon: "🤖", group: "AI 工具" },
    ...
    return (
      <QueryClientProvider client={queryClient}>
        <div className="min-h-screen bg-[#0d1117] text-gray-100">
          ...
          {(tab === "team-register" || tab === "openai-pool" || tab === "vscode" || tab === "turb-register") ? (
            <div className="flex-1 flex flex-col" style={{ height: "calc(100vh - 112px)" }}>
              <iframe
                src={tab === "team-register" ? "/team-all-in-one/" : tab === "vscode" ? "/code/" : tab === "turb-register" ? "/turb-register/" : "/openai-pool/"}
                className="w-full flex-1 border-0"
                title={tab === "team-register" ? "ChatGPT Team 注册面板" : tab === "vscode" ? "VS Code" : tab === "turb-register" ? "GPT 注册" : "OpenAI 账号注册管理"}
                sandbox={tab === "vscode" ? "allow-scripts allow-same-origin allow-forms allow-popups allow-modals allow-downloads allow-top-navigation" : "allow-scripts allow-same-origin allow-forms allow-popups"}
              />
            </div>
          ) : (
