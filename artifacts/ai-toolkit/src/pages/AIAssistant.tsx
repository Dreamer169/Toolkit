...
export default function AIAssistant({ onNavigate }: { onNavigate?: (tab: string) => void } = {}) {
  ...
  const sendToKilo = async () => {
    const task = input.trim();
    if (!task) return;
    try { await navigator.clipboard.writeText(task); } catch {}
    try {
      await fetch("/api/kilo/task", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ task }) });
    } catch {}
    window.open("/code/?folder=/data", "_blank");
  };
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", background:"#0d1117", border:"1px solid #21262d", borderRadius:16, overflow:"hidden", minWidth:0 }}>
        ...
        <div style={{ padding:"8px 14px 12px", borderTop:"1px solid #21262d", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", gap:6, alignItems:"flex-end" }}
            ...
            <button
              onClick={() => void sendToKilo()}
              disabled={!input.trim()}
              title="发送任务给Kilo并打开VS Code"
              style={{ padding:"10px 14px", background: input.trim() ? "#7c3aed" : "#1f2937", border:"none", borderRadius:12, color: input.trim() ? "#fff" : "#4b5563", cursor: input.trim() ? "pointer" : "default", fontSize:13, fontWeight:700, flexShrink:0, transition:"all .2s" }}>
              🤖 Kilo
            </button>
            {loading
