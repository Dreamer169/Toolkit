...
// 2026-05-26: OtherChannel 用户网关独立服务反代。
// /oc/* 保留前缀转发到本机 127.0.0.1:3100，避免落入前端 Vite proxy。
const OC_USER_GATEWAY_TARGET = process.env["OC_USER_GATEWAY_TARGET"] ?? "http://127.0.0.1:3100";
const _ocUserGatewayProxy = createProxyMiddleware({
  target: OC_USER_GATEWAY_TARGET,
  changeOrigin: true,
  ws: true,
  xfwd: true,
  proxyTimeout: 0,
  timeout: 0,
  pathRewrite: (path: string) => `/oc${path}`,
  on: {
    error: (err, _req, res) => {
      logger.warn({ err: String(err) }, "[oc-user-gateway-proxy] upstream :3100 unavailable");
      const r = res as unknown as { headersSent?: boolean; writeHead?: (s: number, h: Record<string, string>) => void; end?: (b: string) => void };
      if (r.writeHead && !r.headersSent) {
        r.writeHead(502, { "content-type": "application/json" });
        r.end?.(JSON.stringify({ error: "otherchannel user gateway unavailable", code: "bad_gateway" }));
      }
    },
  },
});
app.use("/oc", _ocUserGatewayProxy);
