...
## 5. v5.36 变更记录（2026-05-08）

### 核心发现：500 maintenance 是**会话级**错误，非全局维护

> `{"code":500,"msg":"The server is currently being maintained, please try again later~"}`
>
> 这个错误绑定在具体的 chat 实例上。**开新对话（新 chat_id）立即解决**，无需等待。
> v5.36 将这一行为内置进代理：maintenance 自动重试，用户无感知。

### 变更内容

| # | 旧行为 | 新行为（v5.36）|
|---|--------|--------------|
| 1 | 首次 500 → 封锁 service 30 分钟 | 首次 500 → 立即用新 SSID + 新 chat 重试（最多 3 次）|
| 2 | 重试成功概率：0%（被拦在缓存前）| 重试成功概率：高（同一请求内透明恢复）|
| 3 | 用户看到 service_maintenance 报错 | 用户收到正常回复，日志 `[MAINT] maint#1 — retry new chat` |
| 4 | 耗尽重试 → 缓存 30 分钟 | 耗尽重试（真实持续维护）→ 缓存 5 分钟 |

### _try_service 新逻辑

```python
if "service_maintenance" in err or "backend_error_500" in err:
    # 删除了卡住的 chat（GuardedChat finally 已执行）
    # 换下一个 SSID，开全新 chat_id，通常立即恢复
    maint_retries += 1
    if maint_retries > MAX_MAINT_RETRIES:  # MAX_MAINT_RETRIES = 3
        raise  # 确实持续维护，让 _do_chat 缓存 5 分钟
    time.sleep(1.0)
    continue  # 进入下次循环 → 新 SSID + 新 chat_id
```
