...
# 每晚自动跑9平台指纹检测 → 保存结果到本地 → 发邮件告警
# 位置: /root/Toolkit/scripts/nightly_fingerprint_test.sh
# 注意: 结果仅保存在 VPS 本地，不推送到 GitHub
set -euo pipefail
...
<table style='border-collapse:collapse;margin-bottom:16px;'>
  ...
  <tr><td style='padding:4px 14px 4px 0;color:#9ca3af;'>存档</td>
      <td style='color:#6b7280;'>$RESULT_FILE (VPS本地)</td></tr>
</table>
...
<p style='color:#4b5563;font-size:11px;margin-top:20px;'>
  自动生成 by nightly_fingerprint_test.sh @ $(hostname) &mdash; $(ts)
</p>
</body></html>"
...
python3 "$ALERT_SCRIPT" "$SUBJECT" "$BODY_HTML" >> "$RUN_LOG" 2>&1 \
    && log "✅ 邮件发送成功" \
    || log "⚠️  邮件发送失败（结果仍保存在 $RESULT_FILE）"
