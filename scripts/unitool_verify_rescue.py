...
def save_ssid(account_id, email, ssid):
    ...
    # Fix-BUG: guard against error strings being stored as SSIDs
    _INVALID_SSIDS = {"EMAIL_NOT_VERIFIED", "NETWORK_TRANSIENT", "", None}
    if ssid in _INVALID_SSIDS or len(ssid) < 50 or not re.match(r"^[0-9a-f]+$", ssid):
        log(f"[DB] unitool_ssids insert SKIPPED: ssid is not a valid hex token (len={len(ssid) if ssid else 0})")
    else:
        try:
            cur.execute("""
...
def _run_login_once(email, password):
    ...
    display = os.environ.get("DISPLAY") or (":99" if os.path.exists("/tmp/.X11-unix/X99") else ":102")
    env = {**os.environ, "DISPLAY": display, "PYTHONUNBUFFERED": "1"}
    log(f"[login] DISPLAY={display}")
    proc = None
    try:
...
def main():
    ...
    # Fix-8-repoll: login触发新验证邮件 -> 再poll 65s点击 (mirrors inline_verify Fix-C)
    if ssid == "EMAIL_NOT_VERIFIED" and access_token:
        log("[rescue] EMAIL_NOT_VERIFIED -- login may have triggered new verify email, re-poll 65s...")
        _seen_url = verify_url
        _f8_found = ""
        _f8_schedule = [10, 10, 15, 15, 15]
        _f8_elapsed = 0
        for _f8_delay in _f8_schedule:
            import time as _f8t; _f8t.sleep(_f8_delay)
            _f8_elapsed += _f8_delay
            _f8_url = find_verify_link(access_token)
            if _f8_url and _f8_url != _seen_url:
                log("[rescue] Fix-8-repoll new verify email +%ds: %s" % (_f8_elapsed, _f8_url[:80]))
                _f8_found = _f8_url
                break
            log("[rescue] Fix-8-repoll [%ds] no new email" % _f8_elapsed)
        if _f8_found:
            _f8_ssid, _f8_to_entry = click_verify_link(_f8_found)
            log("[rescue] Fix-8-repoll click: ssid=%s to_entry=%s" % ("len="+str(len(_f8_ssid)) if _f8_ssid else "NONE", _f8_to_entry))
            if _f8_ssid:
                ssid = _f8_ssid
            else:
                import time as _f8t2; _f8t2.sleep(5)
                log("[rescue] Fix-8-repoll re-login after new-click...")
                _f8_login = login_via_script(email, password)
                if _f8_login and _f8_login not in ("EMAIL_NOT_VERIFIED", "NETWORK_TRANSIENT"):
                    ssid = _f8_login
                    log("[rescue] Fix-8-repoll re-login OK ssid len=%d" % len(ssid))
                else:
                    log("[rescue] Fix-8-repoll re-login also failed: %s" % str(_f8_login))
        else:
            log("[rescue] Fix-8-repoll no new verify email within 65s -- fallthrough")

    if ssid and ssid not in ("EMAIL_NOT_VERIFIED", "NETWORK_TRANSIENT") and len(ssid) > 50:
