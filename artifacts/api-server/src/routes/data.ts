...
async function runProxyMaintenance() {
  ...
  try {
    ...
    for (let i = 0; i < toCheck.length; i += 5) {
      ...
      const results = await Promise.all(batch.map(async p => ({ id: p.id, ok: await testProxyConnectivity(p.formatted).catch(() => false) })));
...
router.get("/data/unitool-stats", async (req, res) => {
  try {
    // v6: 所有独立数据源并行加载，消除串行等待
    const fs = require("fs") as typeof import("fs");

    // 同步读取文件缓存（微秒级，不阻塞事件循环）
    let convCache: Record<string, { code?: string; conversions?: number; _pool_cache?: { data: Array<{id:number;email:string;ref_code:string;used:number}> } }> = {};
    let tokenCache: Record<string, { regular?: number; bonus?: number; api_ok?: boolean; expires_bonus?: string }> = {};
    let rotateIdx = 0;
    try { convCache = JSON.parse(fs.readFileSync("/tmp/unitool_ref_code_cache.json", "utf8")); } catch {}
    try { tokenCache = JSON.parse(fs.readFileSync("/tmp/unitool_token_cache.json", "utf8")); } catch {}
    try { rotateIdx = Number(JSON.parse(fs.readFileSync("/tmp/unitool_ref_rotate.json", "utf8")).idx ?? 0); } catch {}

    // chain_v3 日志（同步读取，无阻塞）
    let chainStatus = "unknown", chainLastRun = "", chainBrief = "";
    try {
      const logLines = fs.readFileSync("/tmp/unitool_chain_v3_out.log", "utf8").split("\n").filter(Boolean);
      const startLines = logLines.filter((l: string) => l.includes("=== unitool_chain_v3 start ==="));
      if (startLines.length > 0) {
        const m = startLines[startLines.length - 1].match(/\[(\d{2}:\d{2}:\d{2})\]/);
        chainLastRun = m?.[1] ?? "";
      }
      const last20 = logLines.slice(-20);
      if (last20.some((l: string) => l.includes("✅") && l.includes("ssid"))) chainStatus = "success";
      else if (last20.some((l: string) => l.includes("sleep"))) chainStatus = "waiting";
      else if (last20.some((l: string) => l.includes("[main]"))) chainStatus = "running";
      chainBrief = last20
        .filter((l: string) => l.includes("[main]") || l.includes("✅") || l.includes("[ref]") || l.includes("[watermark]"))
        .slice(-4).map((l: string) => l.replace(/^\[\d{2}:\d{2}:\d{2}\]\s*/, "")).join(" | ");
    } catch {}

    // 所有独立网络 / DB 操作并行发出
    const [ol, refRows, deletedRows, recent, failReasons, _poolRaw, _hbRaw] = await Promise.all([
      // 1. Outlook 账号分布
      queryOne<{ fresh:string; registered:string; fail:string; processing:string; total:string }>(
        `SELECT
      ...
      // 2. ref_code 池信息
      query<{ email:string; notes:string; tags:string }>(
      ...
      // 3. 已删除账号 id 列表
      query<{id:number}>("SELECT id FROM accounts WHERE tags LIKE '%unitool_deleted%'", []),
      // 4. 最近5条成功注册
      query<{ id:number; email:string; notes:string; updated_at:string }>(
        `SELECT id, email, notes, updated_at FROM accounts
         WHERE platform='outlook' AND tags LIKE '%unitool_registered%'
         ORDER BY updated_at DESC LIMIT 5`
      ),
      // 5. 24h 失败原因分布（LIKE 全表扫描，并行化消除等待）
      query<{ reason:string; cnt:string }>(
        `SELECT
          CASE
      ...
      new Promise<string>((resolve) => {
        ...
        const r = get({ host:"127.0.0.1", port:8089, path:"/pool-status", timeout:2000 }, (res2) => {
      ...
      new Promise<string>((resolve) => {
        const { get } = require("http") as typeof import("http");
        const r = get({ host:"127.0.0.1", port:8089, path:"/high-balance-status", timeout:2000 }, (res2) => {
          let d = ""; res2.on("data", (c:Buffer) => { d += c; }); res2.on("end", () => resolve(d));
        });
        r.on("error", () => resolve("{}")); r.on("timeout", () => { r.destroy(); resolve("{}"); });
      }),
    ...
    // ── 处理 token 缓存（同步计算）──────────────────────────────────────────
    const deletedIdSet = new Set((deletedRows as Array<{id:number}>).map(r => String(r.id)));
    const activeTokenCache = Object.fromEntries(
      Object.entries(tokenCache).filter(([k]) => !deletedIdSet.has(k))
    );
    const nowMs = Date.now();
    const tokenBonus       = Object.values(activeTokenCache).reduce((s, v) => s + Math.max(0, v.bonus ?? 0), 0);
    const tokenBonusAccts  = Object.values(activeTokenCache).filter(v => (v.bonus ?? 0) > 0).length;
    const tokenBonusZero   = Object.values(activeTokenCache).filter(v => (v.bonus ?? 0) <= 0).length;
    const tokenInitialOnly = Object.values(activeTokenCache).filter(v => { const b = v.bonus ?? 0; return b >= 9.5 && b <= 10.05; }).length;
    const tokenRealHb      = Object.values(activeTokenCache).filter(v => (v.bonus ?? 0) > 10.1).length;
    const tokenRealBonus   = Object.values(activeTokenCache).reduce((s, v) => { const b = v.bonus ?? 0; return s + (b > 10.1 ? b : 0); }, 0);
    const tokenPartial     = Object.values(activeTokenCache).filter(v => { const b = v.bonus ?? 0; return b > 0 && b < 9.5; }).length;
    const tokenExpiredHb   = Object.values(activeTokenCache).filter(v => {
      const b = v.bonus ?? 0; if (b <= 10.1) return false;
      const exp = v.expires_bonus; if (!exp) return false;
      return new Date(exp).getTime() < nowMs;
    }).length;
    const tokenApiFail = Object.values(activeTokenCache).filter(v => v.api_ok === false).length;
    ...
    for (const row of refRows) {
      if ((row.tags ?? "").includes("unitool_ref_master") && !refMasterEmail) refMasterEmail = row.email;
    ...
    const poolCacheData = (convCache as any)._pool_cache?.data as Array<{ref_code:string;used:number;email:string}> ?? [];
    const topPoolEntry = poolCacheData.find((e) => e.used < 10);
    if (topPoolEntry) {
      currentRefCode = topPoolEntry.ref_code; currentRefUsed = topPoolEntry.used;
    } else {
      const availableCodes = codeList.filter(c => c.used < 10);
      const curCode = availableCodes[rotateIdx % Math.max(1, availableCodes.length)];
      currentRefCode = curCode?.code ?? (codeList[0]?.code ?? ""); currentRefUsed = curCode?.used ?? 0;
    }

    // ── 处理 proxy 池状态（来自并行 _poolRaw）──────────────────────────────
    let pool = { total:0, live:0, dead:0, with_balance:0, ssid_len:0 };
    try {
      const pd = JSON.parse(_poolRaw) as { pool_size:number; live:number; accounts?:Array<{dead:boolean;dead_until:number;balance:number|null}> };
      let ssid_len = 0;
      try {
        const ssidFiles = fs.readdirSync("/data/unitool_ssids").filter((f:string) => f.endsWith(".txt"));
        if (ssidFiles.length > 0) ssid_len = fs.readFileSync(`/data/unitool_ssids/${ssidFiles[0]}`).length;
      } catch {}
      const now = Date.now() / 1000;
      const accs = (pd as any).accounts ?? [];
      pool = {
        total: pd.pool_size, live: pd.live,
        dead:         accs.filter((a:any) => a.dead || a.dead_until > now).length,
        with_balance: accs.filter((a:any) => !a.dead && a.dead_until <= now && typeof a.balance === "number" && a.balance > 0).length,
        ssid_len,
      };
    } catch {}

    // ── 处理 high_balance（来自并行 _hbRaw）──────────────────────────────────
    let hbAccounts = 0, hbSsids = 0, hbLive = 0;
    try {
      const hbd = JSON.parse(_hbRaw) as { high_balance_total: number; high_balance_live: number };
      hbAccounts = hbd.high_balance_total ?? 0; hbSsids = hbd.high_balance_total ?? 0; hbLive = hbd.high_balance_live ?? 0;
    } catch {}

    // ── ssid_len（依赖 recent，唯一保留的串行查询）────────────────────────────
    ...
    res.json({
      ...
      ref: { master: refMasterEmail, ref_code: currentRefCode, used: currentRefUsed, limit: 10,
             pool_total: refPoolTotal, pool_available: refPoolAvailable,
             pool_exhausted: refPoolExhausted, total_slots: refTotalSlots },
      pool,
      ...
      recent: recent.map((r: any) => {
        ...
        return {
          ...
          ssid_len:    ssidLenMap[r.email] ?? (m2?.[1]?.length ?? 0),
          updated_at:  r.updated_at,
        };
      }),
      chain:        { status: chainStatus, last_run: chainLastRun, brief: chainBrief },
      fail_reasons: (failReasons as Array<{reason:string;cnt:string}>).map(r => ({ reason: r.reason, count: Number(r.cnt) })),
      token: { total_bonus: tokenBonus, bonus_accounts: tokenBonusAccts, bonus_zero: tokenBonusZero,
               cached_accounts: Object.keys(activeTokenCache).length,
               initial_only: tokenInitialOnly, real_hb: tokenRealHb,
               real_hb_bonus: Math.round(tokenRealBonus * 100) / 100,
               partial_bonus: tokenPartial, expired_hb: tokenExpiredHb, api_fail: tokenApiFail },
      ref_pool_detail: refPoolDetail,
      ts: new Date().toISOString(),
    });
  } catch (e) { res.status(500).json({ success: false, error: String(e) }); }
});

router.get("/data/cf-arkose-stats", (_req, res) => {
  try {
    const { readFileSync } = require("fs");
    const raw = readFileSync("/var/lib/toolkit/cf_arkose_stats.json", "utf8");
    const data: Record<string, { ok: number; fail: number }> = JSON.parse(raw);
    const subnets = Object.entries(data).map(([subnet, v]) => {
      const ok = v.ok ?? 0;
      const fail = v.fail ?? 0;
      const total = ok + fail;
      const rate = total > 0 ? Math.round((ok / total) * 1000) / 10 : 0;
      return { subnet, ok, fail, total, rate };
    });
    subnets.sort((a, b) => b.total - a.total);
    res.json({ success: true, subnets, ts: new Date().toISOString() });
  } catch (e: any) {
    res.json({ success: false, subnets: [], error: e.message });
  }
});
