#!/usr/bin/env python3
"""
proxy_manager.py v1.0 -- Unified Proxy Manager
...
class ProxyManager:
            ...
            try:
                ...
                for p in items:
                    ...
                    if ex is None:
                        ...
                        changed = False
                        if p.get("username") and ex.user != p["username"]:
                            ex.user = p["username"]; ex.passwd = p.get("password","")
                            changed = True
    ...
    def probe_all(self, max_workers: int = 20, force: bool = False,
        ...
        entries = [e for e in self.db.all()
        ...
        for e in all_e:
            ...
            if s not in by_source:
                by_source[s] = {"total":0,"alive":0,"dead":0,
                                "unknown":0,"expired":0,"blacklisted":0,
                                "countries": []}
