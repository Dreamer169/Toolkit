...
class KiroRegister:
    ...
    def step2_get_wsh(self,redir_url):
        self.log("Step 2: redirect chain + portal.sso...")
        # --- Phase A: follow redirect chain, extract wsh ---
        current_url = redir_url
        wsh_from_chain = None
        for hop in range(14):
            try:
                r = self.s.get(current_url,
                               headers={**UA,"accept":"text/html,application/xhtml+xml,*/*"},
                               allow_redirects=False)
            except Exception as e:
                self.log("hop"+str(hop)+" failed: "+str(e)); break
            hop_url = str(r.url)
            location = r.headers.get("location") or r.headers.get("Location","")
            self.log("  hop"+str(hop)+": status="+str(r.status_code)+" url="+hop_url[:100])
            if location: self.log("    loc="+location[:120])
            self._capture_cookies(r)
            # Check for wsh in any URL in this hop
            for candidate in [hop_url, location]:
                if not candidate: continue
                m = re.search(r"workflowStateHandle=([^&#]+)", candidate)
                if m and not wsh_from_chain:
                    wsh_from_chain = m.group(1)
                    self.log("  hop"+str(hop)+" wsh="+wsh_from_chain[:50]+"...")
            if r.status_code in (301,302,303,307,308) and location:
                if location.startswith("http"):
                    current_url = location
                else:
                    _b = urlparse(current_url)
                    current_url = _b.scheme+"://"+_b.netloc+location
                continue
            break
        view_url = current_url
        self.log("  2a view: "+view_url[:120])
        # --- Phase B: portal.sso to get csrfToken ---
        # We don't have orchestrator_id yet; use a placeholder view.awsapps.com URL.
        # portal.sso will still return a csrfToken for CSRF protection.
        self._orchestrator_id = ""  # will be updated by step12 from sso_redirect
        self._callback_url = ""
        vr = "https://view.awsapps.com/start/#/"
        vr_enc = url_quote(vr)
        pu = "https://portal.sso.us-east-1.amazonaws.com/login?directory_id=view&redirect_url="+vr_enc
        self.log("  2b portal.sso (get csrfToken)...")
        try:
            ...
            self.log("  Status: "+str(r2.status_code))
            try:
                d=r2.json()
                csrf=d.get("csrfToken")
                if csrf:
                    self._portal_csrf_token=str(csrf)
                    self.log("  portal csrfToken="+self._portal_csrf_token[:40]+"...")
                # Also check redirectUrl for orchestrator_id
                redirect_url = d.get("redirectUrl","")
                if redirect_url:
                    _p = urlparse(redirect_url)
                    _qs = parse_qs(_p.query)
                    _fqs = parse_qs(_p.fragment.lstrip("#/?")) if _p.fragment else {}
                    _oid = (_qs.get("orchestrator_id") or _fqs.get("orchestrator_id",[None]))[0]
                    if _oid:
                        self._orchestrator_id = _oid
                        self.log("  orchestrator_id="+_oid[:60]+"...")
            except Exception:
                self.log("  portal.sso response not JSON, status="+str(r2.status_code))
        except Exception as e2:
            self.log("  portal.sso request failed: "+str(e2))
        # --- Phase C: set wsh ---
        if wsh_from_chain:
            self.wsh = wsh_from_chain
            self._login_wsh = wsh_from_chain
            self.log("  wsh from redirect chain: "+wsh_from_chain[:50]+"...")
            # Load the actual login page to set cookies
            try:
                r3 = self.s.get(view_url,headers={**UA,"accept":"text/html",
                    "referer":"https://portal.sso.us-east-1.amazonaws.com/"},
                    allow_redirects=True)
                self._capture_cookies(r3)
            except Exception as e3:
                self.log("  login page load err: "+str(e3))
            self._setup_signin_js_cookies()
            return True
        # Fallback: check portal.sso redirectUrl for wsh
        self.log("  no wsh from redirect chain")
        return False
    ...
    # ═══ register(): 全流程编排 (step1 → step12f) ═══
    def register(self, email, pwd=None, name=None, mail_token=None):
        """编排全部注册步骤. 返回 (ok:bool, info:dict)."""
        import time as _time
        password = pwd or _pwd()
        full_name = name or f"Kiro User {_uuid()[:6]}"

        self.log(f"=== register({email}) 开始 ===")

        # Step 1: InitiateLogin
        redir_url = self.step1_kiro_init()
        if not redir_url:
            return False, {"error": "step1 失败: 无 redirectUrl"}

        # Step 2: 获取 workflowStateHandle
        if not self.step2_get_wsh(redir_url):
            return False, {"error": "step2 失败: 无 wsh"}

        # Step 3: signin → SIGNUP
        r3 = self.step3_signin_flow(email)
        if r3 is None:
            return False, {"error": "step3 失败"}

        # Step 4: signup → profile.aws redirect
        r4 = self.step4_signup_flow(email)
        if r4 is None:
            return False, {"error": "step4 失败"}

        # Step 5: TES token
        if not self.step5_get_tes_token():
            return False, {"error": "step5 失败: 无 TES token"}

        # Step 6: profile.aws 加载
        r6 = self.step6_profile_load()
        if r6 is None:
            return False, {"error": "step6 失败"}

        # Step 7: 发送 OTP
        r7 = self.step7_send_otp(email)
        if r7 is None:
            return False, {"error": "step7 失败: OTP 发送失败"}

        # 等待 OTP (调用模块级 wait_for_otp, 可被外部猴子补丁替换)
        import sys as _sys
        _mod = _sys.modules.get(__name__) or _sys.modules.get("kiro_core")
        otp_fn = getattr(_mod, "wait_for_otp", None) if _mod else None
        if otp_fn is None:
            # 回退: 使用全局命名空间中的函数
            otp_fn = globals().get("wait_for_otp")
        if otp_fn is None:
            return False, {"error": "wait_for_otp 未找到"}

        otp = otp_fn(account_id=mail_token, timeout=120, tag=self.tag)
        if not otp:
            return False, {"error": "step8 前置失败: OTP 超时"}

        # Step 8: create-identity (OTP 验证)
        r8 = self.step8_create_identity(otp, email, full_name)
        if r8 is None:
            return False, {"error": "step8 失败"}
        reg_code = r8.get("registrationCode")
        sign_in_state = r8.get("signInState")
        if not reg_code or not sign_in_state:
            return False, {"error": "step8 无 registrationCode/signInState"}

        # Step 9: signup execute (registrationCode → get-new-password)
        r9 = self.step9_signup_registration(reg_code, sign_in_state)
        if r9 is None:
            return False, {"error": "step9 失败"}

        # Step 10: 设置密码 (JWE 加密)
        r10 = self.step10_set_password(password, email, r9)
        if r10 is None:
            return False, {"error": "step10 失败: 密码设置失败"}

        # Step 11: 最终登录
        r11 = self.step11_final_login(email, r10)
        if r11 is None:
            return False, {"error": "step11 失败"}
