...
function injectIntoPool(ips) {
  ...
  const newEntries = candidates.map(ip => ({ ip, latency: 9999.0, proxy: `https://${ip}:443` }));  // latency=9999 = unverified, retest will measure real values
...
async function main() {
  ...
  if (outbounds.length > 0) {
    ...
    fs.renameSync(TMP, CFG);  // atomic replace, prevent xray reading partial JSON
    ...
    reloadXray();
    // 立即将新 IP 推送到 KV（不等 cron 每小时一次）
    try {
      const { execSync: _ex } = require("child_process");
      _ex("/data/push_cfips_to_kv.sh >> /tmp/toolkit_logs/kv_sync.log 2>&1", { shell: true, timeout: 60000 });
      log("✅ KV 同步完成（xray更新后立即触发）");
    } catch (kvErr) {
      log("⚠️ KV 推送失败（非致命）: " + kvErr.message);
    }
  } else {
