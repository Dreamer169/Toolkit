...
http.createServer((q,s)=>{s.setHeader("Content-Type","application/json");if(q.url==="/"||q.url==="/health"){s.writeHead(200);s.end(JSON.stringify({ok:true,name:NAME,openai:Boolean(OBASE&&OKEY),anthropic:Boolean(ABASE&&AKEY),gemini:Boolean(GBASE&&GKEY),time:new Date().toISOString()}));return;}if(q.url.startsWith("/v1/")){if(OBASE&&OKEY)return proxy(q,s,OBASE,OKEY,"");s.writeHead(503);s.end(JSON.stringify({error:"AI_INTEGRATIONS_OPENAI 未配置"}));return;}if(q.url.startsWith("/anthropic/")){if(ABASE&&AKEY)return proxy(q,s,ABASE,AKEY,"/anthropic");s.writeHead(503);s.end(JSON.stringify({error:"AI_INTEGRATIONS_ANTHROPIC 未配置"}));return;}s.writeHead(404);s.end('{"error":"nf"}');}).listen(PORT,"0.0.0.0",()=>{console.log("[agent] port="+PORT+" gw="+GW);reg();setInterval(reg,5*60*1000);});
