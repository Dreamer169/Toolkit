...
// ── recover-suspended: 批量验证 suspended+abuse_mode 账号，回收误判 ──────────
let _recoverRunning = false;
let _recoverStats: {
  total: number; checked: number; recovered: number; confirmed: number; skipped: number;
  finishedAt: string | null;
} = { total: 0, checked: 0, recovered: 0, confirmed: 0, skipped: 0, finishedAt: null };

async function runRecoverSuspended(): Promise<void> {
  if (_recoverRunning) return;
  _recoverRunning = true;
  let checked = 0, recovered = 0, confirmed = 0, skipped = 0;
  try {
    const accounts = await query<{ id: number; email: string; token: string }>(
    ...
    for (let i = 0; i < accounts.length; i += BATCH) {
      ...
      await Promise.all(batch.map(async (acc) => {
        ...
        try {
          ...
          if (gr.ok) {
            // 旧 token 仍有效：是误判的 suspended，恢复 active，清 abuse_mode
            recovered++;
            await execute(
            ...
            confirmed++; // Graph 403 确认真封禁，不动
          } else {
            skipped++; // 401/网络 → token过期但不确定封禁，跳过
          }
        } catch { skipped++; }
        _recoverStats = { total: _recoverStats.total, checked, recovered, confirmed, skipped, finishedAt: null };
      }));
      await new Promise(r => setTimeout(r, 500)); // 批次间隔，避免 rate limit
    }
  } catch (e) {
    logger.error({ err: String(e) }, "[recover-suspended] 出错");
  } finally {
    _recoverRunning = false;
    _recoverStats = { ..._recoverStats, finishedAt: new Date().toISOString() };
    logger.info(_recoverStats, "[recover-suspended] 完成");
  }
}

router.post("/tools/outlook/recover-suspended", (_req, res) => {
  if (_recoverRunning) {
...
router.get("/tools/outlook/recover-suspended/status", (_req, res) => {
