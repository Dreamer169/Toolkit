...
async function reg(){if(!ME)return;try{const r=await post(GW+"/api/gateway/self-register",JSON.stringify({gatewayUrl:ME,name:NAME,openaiBaseUrl:OBASE,openaiApiKey:OKEY,anthropicBaseUrl:ABASE,anthropicApiKey:AKEY}));console.log("[agent] reg:",r.slice(0,80));}catch(e){console.error("[agent]",e.message);}}
http.createServer((q,s)=>{s.setHeader("Content-Type","application/json");if(q.url==="/"||q.url==="/health"){s.writeHead(200);s.end(JSON.stringify({ok:true,name:NAME,time:new Date().toISOString()}));}else{s.writeHead(404);s.end('{"error":"nf"}');}}).listen(PORT,"0.0.0.0",()=>{console.log("[agent] port="+PORT+" gw="+GW);reg();setInterval(reg,5*60*1000);});
