...
MIN_REF_INTERVAL = 300  # v3.6: 同一 ref_code 两次使用最短间隔(秒)，防 unitool 速率过滤
RESI_PORTS = [10853, 10854, 10856, 10857, 10859]  # Fix-10: verified alive 2026-05-23
...
def _api_check_ref_code(ssid: str, account_id: int = 0) -> tuple:
    ...
    try:
        ...
        try:
            ...
            return ("", -1)
        if raw == "null":
            # v5.16 fix: null = 账号没有创建 ref_code（cfmail 正常情况）≠ SSID 失效
            # 旧逻辑 v5.15 直接 mark is_valid=FALSE → 误杀有效账号的 SSID
            # 正确处理：不 invalidate，仅当 /api/auth/session 也失效时才 mark dead
            try:
                _sc_cmd = [
                    "curl", "-s", "--socks5-hostname", f"127.0.0.1:{_rc_port}",
                    "-b", f"__Secure-unitool-ssid={ssid}",
                    "-H", "Accept: application/json", "--max-time", "8",
                    "https://unitool.ai/api/auth/session",
                ]
                _sc_proc = subprocess.Popen(_sc_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                _sc_out, _ = _sc_proc.communicate(timeout=12)
                _sc_raw = _sc_out.decode("utf-8", errors="ignore").strip()
                _sc_data = json.loads(_sc_raw) if _sc_raw and _sc_raw != "null" else {}
                _sc_user = (_sc_data.get("auth") or {}).get("user") or {}
                if not _sc_user.get("id"):
                    # Session really expired → mark invalid
                    _inv_c = db_connect(); _inv_cur = _inv_c.cursor()
                    _inv_cur.execute("UPDATE unitool_ssids SET is_valid=FALSE WHERE ssid=%s", (ssid,))
                    _inv_c.commit(); _inv_c.close()
                    log(f"[SSID] is_valid=FALSE (session confirmed expired) id={account_id}")
                else:
                    log(f"[ref] null ref-code but session OK id={account_id} → 账号无 ref_code（cfmail/未创建）")
            except Exception as _inv_e:
                log(f"[ref] null ref-code double-check err: {_inv_e}")
...
def create_ref_code_via_proxy(ssid: str, email: str, port_hint: int = 0) -> str:
    ...
    def _tor_newnym_silent():
        try:
            import subprocess as _sp2
            _sp2.run(["python3", "/data/Toolkit/scripts/tor_newnym.py"],
                     capture_output=True, timeout=8)
        except Exception:
            pass
    for _tor_port in _TOR_PORTS:
        try:
            _tor_newnym_silent()  # v9.46: 每次前换IP，避免用已记录IP
    ...
    try:
        ...
        if os.path.exists(_ext_file):
            ...
            for _ext_proxy in _ext_list[:3]:  # v9.46: 只试3个，快速fallback到QuarkIP
                try:
        ...
        for entry in data:
            if entry.get("ref_code") == used_ref_code:
                ...
                if isinstance(ind_conv, int) and ind_conv >= 0 and new_used > ind_conv + 3:
                    ...
                    new_used = ind_conv + 1
                entry["used"] = new_used
                entry["last_used_ts"] = time.time()  # v3.6: 速率控制时间戳
                updated = True
...
def pick_rotating_ref_code(pool: list) -> dict | None:
    ...
    v3.6: 跳过 MIN_REF_INTERVAL 秒内刚刚用过的码（防 unitool 速率过滤），
    自动顺延到下一个冷却完毕的码。
    """
    if not pool:
        return None
    ...
    for idx, candidate in enumerate(pool):
        last_ts = candidate.get("last_used_ts", 0)
        cooldown_remaining = MIN_REF_INTERVAL - (now - last_ts)
        if cooldown_remaining > 0:
            log(f"[ref] skip #{idx+1} ref_code={candidate['ref_code']} "
                f"冷却中 {cooldown_remaining:.0f}s")
            continue
        chosen = candidate
        log(f"[ref] 填满优先 #{idx+1}/{len(pool)}: ref_code={chosen['ref_code']} "
            f"from {chosen['email']} used={chosen['used']}/{MAX_REF_SLOTS}")
        return chosen
    # 所有码都在冷却中 → 选冷却时间最短的等待
    soonest = min(pool, key=lambda x: x.get("last_used_ts", 0))
    wait_sec = MIN_REF_INTERVAL - (now - soonest.get("last_used_ts", 0))
    wait_sec = max(0, int(wait_sec))
    log(f"[ref] 所有 ref_code 冷却中，最快 {wait_sec}s 后可用: {soonest['ref_code']} → 等待")
    time.sleep(wait_sec + 1)
    log(f"[ref] 冷却完毕，选用: {soonest['ref_code']}")
    return soonest
...
def run_inline_verify(email: str, password: str, refresh_token: str,
    ...
    try:
        ...
        # 存新 refresh_token — MS 每次返回新 token，不存会逐渐失效
        _new_rt = _resp.get("refresh_token", "")
        if _new_rt and _new_rt != refresh_token and account_id:
            try:
                _rt_c = db_connect(); _rt_cur = _rt_c.cursor()
                _rt_cur.execute("UPDATE accounts SET refresh_token=%s, updated_at=NOW() WHERE id=%s",
                                (_new_rt, account_id))
                _rt_c.commit(); _rt_c.close()
                log(f"[inline_verify] 新 refresh_token 已存回 DB id={account_id}")
                refresh_token = _new_rt  # 本次调用使用新 token
            except Exception as _rte:
                log(f"[inline_verify] refresh_token 存储失败(非致命): {_rte}")
...
def run_cfmail_chain(ref_code):
    ...
    if not reg["ok"]:
        ...
        # Fix-8: no_verify_email / verify_pending = step1 (form submit) succeeded,
        # unitool_register.py tried MS Graph inbox (wrong for cfmail) and failed.
        # The verification email IS in cfmail D1 — proceed to _cfmail_poll_d1.
        _step1_ok = any(p in reason_l for p in (
            "no_verify_email", "verify_pending", "verify_email"))
        if not _step1_ok:
            log("[cfmail] reg fail (hard): " + reason)
            if account_id:
                db_mark_fail(account_id, reason)
            return {"ok": False, "email": email, "ssid": "", "reason": reason}
        log("[cfmail] step1 OK but inbox-verify failed (expected for cfmail domain) -> D1 poll")
    else:
    ...
    persist_ssid(email, ssid)
    # Fix-9b: email-verify SSID has session.active=false -> upgrade via run_login in background
    # Non-blocking: spawn thread so chain is not delayed by 180s login timeout
    _pw9 = "Unitool@2024!"
    _aid9 = account_id
    if _aid9:
        try:
            _c9 = db_connect(); _cu9 = _c9.cursor()
            _cu9.execute("SELECT password FROM accounts WHERE id=%s", (_aid9,))
            _r9 = _cu9.fetchone(); _c9.close()
            if _r9 and _r9[0]: _pw9 = _r9[0]
        except Exception: pass
    def _fix9_login_thread(em, pw, aid):
        try:
            log("[cfmail] Fix-9b: bg-login start " + em)
            new_ssid = run_login(em, pw)
            if new_ssid:
                if aid: db_save_ssid_full(aid, em, new_ssid)
                persist_ssid(em, new_ssid)
                log("[cfmail] Fix-9b: active SSID saved len=" + str(len(new_ssid)))
            else:
                log("[cfmail] Fix-9b: login failed, verify-SSID stays (session.active=false)")
        except Exception as _e9:
            log("[cfmail] Fix-9b: bg-login err: " + str(_e9))
    import threading as _t9
    _t9.Thread(target=_fix9_login_thread, args=(email, _pw9, _aid9), daemon=True).start()
    log("[cfmail] Fix-9b: bg-login thread spawned for " + email)
...
def main():
    ...
    if not row:
        ...
        if cf_result["ok"]:
            ...
            # v3.6: cfmail 也消耗 ref_code slot → 同步本地计数，防止 pool 超量复用
            _pool_cache_increment_used(ref_code)
            if ref_master_id:
                try:
                    _cf_conn = db_connect(); _cf_cur = _cf_conn.cursor()
                    _cf_cur.execute(
                        "UPDATE accounts SET notes=COALESCE(notes,'')||%s, updated_at=NOW() WHERE id=%s",
                        (f"\nref_registered={cf_result['email']}|id=cfmail", ref_master_id))
                    _cf_conn.commit(); _cf_conn.close()
                    log(f"[ref] cfmail tracked: master({ref_master_email}) +1 → {cf_result['email']}")
                except Exception as _cfe:
                    log(f"[ref] cfmail track err: {_cfe}")
            return
    ...
    if ref_master_id:
        try:
            ...
            if _hb_count >= MAX_REF_SLOTS and "unitool_high_balance" not in _hb_tags:
                # v3.5: 升入 HB 前先实时调 API 核验 conversions，防止本地多计误升
                _hb_ssid = ""
                _ssid_m = re.search(r"unitool_ssid=([0-9a-f]{40,})", _hb_notes)
                if _ssid_m:
                    _hb_ssid = _ssid_m.group(1)
                _api_conv = -1
                if _hb_ssid:
                    # 清掉缓存条目，强制拿最新值
                    try:
                        _fc = _load_ref_cache()
                        _fc.pop(str(ref_master_id), None)
                        _save_ref_cache(_fc)
                    except Exception:
                        pass
                    _, _api_conv = _api_check_ref_code(_hb_ssid, ref_master_id)
                    log(f"[ref] HB pre-check: local={_hb_count} API conv={_api_conv}/{MAX_REF_SLOTS}")
                else:
                    log(f"[ref] HB pre-check: no SSID，trust local={_hb_count}")

                if _api_conv >= 0 and _api_conv < MAX_REF_SLOTS:
                    # API 未达标 → 暂不升级，等下一次注册再触发
                    log(f"[ref] ref_master({ref_master_email}) local={_hb_count}>={MAX_REF_SLOTS} "
                        f"但 API conv={_api_conv}<{MAX_REF_SLOTS} → 暂缓升 HB，等 API 追上")
                else:
                    # API >= MAX_REF_SLOTS 或 API 调用失败(conv=-1)时信任本地
                    if _api_conv >= MAX_REF_SLOTS:
                        log(f"[ref] API conv={_api_conv}>={MAX_REF_SLOTS} ✓ 确认升入 HB")
                    else:
                        log(f"[ref] API check 失败(conv={_api_conv})，信任本地 local={_hb_count} → 升入 HB")
                    ...
                    log(f"[ref] 🎉 ref_master({ref_master_email}) "
                        f"ref×{_hb_count}/{MAX_REF_SLOTS} API×{_api_conv} → unitool_high_balance 高余额池")
                    try:
