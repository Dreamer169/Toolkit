// fp_quick.mjs 风格：极简 init（无 STEALTH_INIT）→ 验证 251283a 的 0%/0%/44% 基线是否还能复现
...
await ctx.addInitScript(`(function(){
  ...
  try {
    ...
    _c.runtime={id:undefined,lastError:null,onConnect:_mk(),onConnectExternal:_mk(),onMessage:_mk(),onMessageExternal:_mk(),onInstalled:_mk(),onStartup:_mk(),onSuspend:_mk(),onSuspendCanceled:_mk(),onUpdateAvailable:_mk(),onRestartRequired:_mk()};
    ...
    _c.app=_c.app||{isInstalled:false,getDetails(){return null;},getIsInstalled(){return false;}};
  } catch(_e){}
...
console.log('Going to CreepJS with MINIMAL init (fp_quick.mjs style)...');
...
const result = await page.evaluate(() => {
  ...
  const lines = body.split('\n').map(l=>l.trim()).filter(l=>
    l && /headless|stealth|like headless|Grade|Worker|SharedWorker/i.test(l)
  );
  const grade = document.querySelector('[class*="grade"]')?.textContent?.trim() || '';
  return { lines: lines.slice(0,30), grade };
});

console.log('=== CreepJS (MINIMAL init / no STEALTH_INIT) ===');
