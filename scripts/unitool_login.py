...
async def _bypass_turnstile(tab, label="", timeout=55) -> bool:
    """
    Turnstile bypass v5.0 — Invisible + Managed hybrid (mirrors http_register v5.0)
    Phase 1: 30s natural token wait (Invisible auto-solve for good RESI IPs)
    Phase 2: _bypass_cloudflare() click (for degraded/blocked IPs)

    Root cause: CF Invisible grades IP reputation live.
    Fresh IPs -> token ~30s. Reused IPs -> no token -> need click.
    """
    ...
    async def _poll(seconds):
        for _ in range(seconds):
            ...
            if n > 20:
                return await _get_full_token(tab)
            try:
    ...
    # Phase 1: Invisible auto-solve (30s)
    _log(f"    [{label}] [phase1] natural token 30s...")
    tok = await _poll(30)
    if tok:
        _log(f"    [{label}] ✓ natural token len={len(tok)}")
        return True

    # Phase 2: active click
    _log(f"    [{label}] [phase2] _bypass_cloudflare() click")
    for attempt in range(3):
        try:
            await tab._bypass_cloudflare({}, time_to_wait_captcha=15)
            _log(f"    [{label}] bypass click attempt={attempt+1} OK")
        except Exception as e:
            _log(f"    [{label}] bypass attempt={attempt+1} err: {e}")
        tok = await _poll(12)
        if tok:
            _log(f"    [{label}] ✓ managed token attempt={attempt+1} len={len(tok)}")
            return True
        await asyncio.sleep(2)

    _log(f"    [{label}] ✗ all phases failed")
    return False
...
async def login_one(email: str, password: str, headless: bool = True,
    ...
    opt.start_timeout = 90  # 并发/残留Chrome时启动慢，默认45s仍会偶发失败
