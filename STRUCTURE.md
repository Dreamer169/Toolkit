...
| 文件 | 说明 |
|------|------|
| `browser-model/artifacts/api-server/test_9platforms_v2.mjs` | 9平台综合检测 v2（Asia/Hong_Kong，IPHey 轮询90s，FP visitor ID）|
| `browser-model/artifacts/api-server/test_iphey_fp_v2.mjs` | IPHey + Fingerprint.com 专项诊断 |
| `browser-model/artifacts/api-server/test_extra_platforms.mjs` | 扩展3平台: incolumitas.com / deviceandbrowserinfo.com / f.vision |
| `browser-model/artifacts/api-server/test_tz_diversity.mjs` | 5代理端口×时区 IPHey 一致性测试 |
| `browser-model/artifacts/api-server/test_load_concurrent.mjs` | 生产负载压测: 并发N个browser-model实例，观察内存/CPU/通过率 |

### 运维自动化

| 文件 | 说明 |
|------|------|
| `scripts/nightly_fingerprint_test.sh` | 每晚 3:00 cron → 跑测试 → 保存本地 → 发邮件 |
| `scripts/send_alert.py` | Outlook 告警发件：遍历 DB 有效账号，目标 rjrbaphnak@rommiui.com |

### 测试结果归档

- 路径：`docs/test-results/YYYY-MM-DD.log`（VPS 本地，不推送 GitHub）
- 保留最近 30 天，每天 04:00 自动清理

### Cron 配置 (crontab -l 节选)

```
0 3 * * *   /root/Toolkit/scripts/nightly_fingerprint_test.sh >> /tmp/toolkit_logs/nightly_fp_cron.log 2>&1
0 4 * * *   find /root/Toolkit/docs/test-results/ -name "*.log" -mtime +30 -delete 2>/dev/null; find /tmp/toolkit_logs/ -name "fp_run_*.tmp" -mtime +7 -delete 2>/dev/null
*/5 * * * * pgrep -f "Xvfb :99" > /dev/null || (rm -f /tmp/.X99-lock /tmp/.X99-unix/.X99 2>/dev/null; pm2 restart xvfb 2>/dev/null)
*/5 * * * * ss -tlnp 2>/dev/null | grep -q ":8092" || (sleep 2; ss -tlnp 2>/dev/null | grep -q ":8092" || pm2 restart browser-model 2>/dev/null)
```
...
- IPHey：轮询直到 "Temporary value" 消失，最长 90s
- Fingerprint.com：等待 32 位十六进制 visitor ID 生成即为 PASS

## _PROXY_PORT_TZ 完整映射表

代理端口 → 时区 → UTC偏移（std/dst），由 `renderer.ts: _PROXY_PORT_TZ` 定义。

| 端口 | 时区 | UTC偏移(标准) | UTC偏移(夏令) | 夏令时 |
|------|------|:---:|:---:|:---:|
| 10851 | America/New_York    | -5h | -4h | ✅ |
| 10853 | America/Los_Angeles | -8h | -7h | ✅ |
| 10854 | Asia/Seoul          | +9h | +9h | ❌ |
| 10855 | Europe/London       | +0h | +1h | ✅ |
| 10857 | Asia/Hong_Kong      | +8h | +8h | ❌ |
| 10859 | Europe/Amsterdam    | +1h | +2h | ✅ |
| 10910 | America/Los_Angeles | -8h | -7h | ✅ |
| 10911 | America/Los_Angeles | -8h | -7h | ✅ |
| 10912 | America/Los_Angeles | -8h | -7h | ✅ |
| 10914 | Europe/London       | +0h | +1h | ✅ |
| 10915 | America/Mexico_City | -6h | -5h | ✅ |
| 10916 | America/Los_Angeles | -8h | -7h | ✅ |

**默认fallback**：无匹配端口 → `America/Los_Angeles`
...
**问题**: 两服务均处于高频重启循环
- **xvfb**: 孤儿锁文件 `/tmp/.X99-lock` 残留 → 每次 pm2 重启失败 `Server is already active for display 99`
- **browser-model**: 孤儿 node 进程(pid=3126610)占用 `:8092` → pm2 重启均 EADDRINUSE
- **修复**: kill 孤儿进程 → 清锁文件 → pm2 restart；cron watchdog 防止复发
