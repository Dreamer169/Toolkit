...
type SkillId = "cursor" | "outlook" | "mail" | "tokens" | "data" | "monitor" | "workflow" | "pool" | "captcha" | "waf-bypass" | "sms" | "webshare";
...
add({ name: "outlook.imap.enable", label: "开启 IMAP 访问", description: "为 Outlook 账号开启 IMAP 并处理微软安全验证（密码重验 + 备用邮箱 + 验证码）", skillId: "outlook", method: "POST", path: "/api/tools/outlook/enable-imap", safety: "requires-confirmation", parameters: [{ name: "account_id", type: "number" as ParamType, description: "Outlook 账号 ID", required: false }], tab: "outlook", keywords: ["imap", "开启", "enable", "邮箱", "设置"] });
add({ name: "outlook.imap.enableBatch", label: "批量开启 IMAP", description: "批量为多个 Outlook 账号开启 IMAP", skillId: "outlook", method: "POST", path: "/api/tools/outlook/enable-imap/batch", safety: "requires-confirmation", parameters: [param.ids], tab: "outlook", keywords: ["imap", "批量", "开启", "enable"] });
