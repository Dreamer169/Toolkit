...
async function processMessage(
  ...
  // proxy: null → Graph API 直连 graph.microsoft.com（可达）
  // replit_browser_proxy: Tor 9050 — 实测 replit.com 200 ✅；不做全局路由，不影响 SSH
  // WARP(40000) 从未安装(v9.43)，已废弃
  const params = JSON.stringify({ token: accessToken, message_id: msg.id, verify_url: "", proxy: null, replit_browser_proxy: "socks5://127.0.0.1:9050" });
