...
const RETEST_INTERVAL_MS = 5 * 60 * 1000; // 每5min重测连通性（与老服务器一致）
...
function retestPool(): void {
  if (_refreshRunning) return; // 补充中跳过，避免并发
  logger.info("[cf-pool] 开始定期连通性重测");
  const r = spawnSync("python3", [CF_POOL_SCRIPT, "retest",
    "--max-latency", "800", "--threads", "8", "--port", "443",
  ], { timeout: 90_000, encoding: "utf8", env: { ...process.env, PYTHONUNBUFFERED: "1" } });
  try {
    const d = JSON.parse(r.stdout || "{}") as Record<string, unknown>;
    logger.info({ kept: d["kept"], removed: d["removed"] }, "[cf-pool] 连通性重测完成");
  } catch {
    logger.warn({ stderr: r.stderr?.slice(0, 200) }, "[cf-pool] 连通性重测解析失败");
  }
}
...
export function startCfPoolMaintainer() {
  ...
  // 启动后5min做首次连通性重测，之后每30min一次
  setTimeout(() => { retestPool(); }, 5 * 60_000);
  setInterval(() => { retestPool(); }, RETEST_INTERVAL_MS);
}
