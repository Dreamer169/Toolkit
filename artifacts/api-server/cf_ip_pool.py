...
def refresh_pool(generate_count: int = 60, target_valid: int = 20, threads: int = 5, port: int = 443, max_latency: float = 9999.0, log_cb=None) -> list:
    ...
    # v9.99: sync fresh IPs into xray VLESS outbounds
    if new_ips:
        _fresh = [x["ip"] for x in new_ips]
        threading.Thread(target=update_xray_cf_ips, args=(_fresh,), daemon=True).start()
    return new_ips
...
# ── xray.json 动态 CF IP 更新（v9.99+） ───────────────────────────────────
XRAY_JSON_PATH = '/data/Toolkit/xray.json'

def update_xray_cf_ips(new_ips: list) -> int:
    '''用新测通的 CF IP 更新 xray.json 里所有 proxy-N VLESS 出站地址，然后 pm2 restart xray。
    这样 gratis-register proxy_pool.py 使用的 socks5h://127.0.0.1:10820~10889
    就会经过更新过的 CF IP -> CF Worker -> ia.gratis 路由。
    返回更新的出站数量，0 表示失败或无变化。'''
    if not new_ips:
        return 0
    import fcntl, tempfile, shutil, subprocess, copy
    lock_path = XRAY_JSON_PATH + '.lock'
    try:
        with open(lock_path, 'a') as lf:
            fcntl.flock(lf, fcntl.LOCK_EX)
            try:
                with open(XRAY_JSON_PATH) as f:
                    cfg = json.load(f)
                vless_outs = [ob for ob in cfg.get('outbounds', [])
                               if ob.get('protocol') == 'vless'
                               and ob.get('tag', '').startswith('proxy-')]
                if not vless_outs:
                    return 0
                updated = 0
                for i, ob in enumerate(vless_outs):
                    new_ip = new_ips[i % len(new_ips)]
                    vnext = ob.get('settings', {}).get('vnext', [])
                    if vnext and vnext[0].get('address') != new_ip:
                        vnext[0]['address'] = new_ip
                        updated += 1
                if updated == 0:
                    return 0
                fd, tmp = tempfile.mkstemp(dir=os.path.dirname(XRAY_JSON_PATH), suffix='.tmp')
                with os.fdopen(fd, 'w') as f:
                    json.dump(cfg, f, indent=2)
                shutil.move(tmp, XRAY_JSON_PATH)
                subprocess.run(['pm2', 'restart', 'xray'], check=False,
                                capture_output=True, timeout=10)
                print(f'[cf_pool] xray CF IPs updated: {updated} outbounds -> {new_ips[:6]}', flush=True)
                return updated
            finally:
                fcntl.flock(lf, fcntl.LOCK_UN)
    except Exception as e:
        print(f'[cf_pool] update_xray_cf_ips failed: {e}', flush=True)
        return 0
