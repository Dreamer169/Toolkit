...
def poll_inbox_for_verify(refresh_token, timeout_sec=300, interval_sec=20, account_id=0):
    ...
    while time.time() < deadline:
        try:
            if not access_token:
                ...
                _new_rt = resp.get("refresh_token", "")
                if _new_rt and _new_rt != refresh_token and account_id:
                    try:
                        _rc = db_connect(); _rcur = _rc.cursor()
                        _rcur.execute("UPDATE accounts SET refresh_token=%s, updated_at=NOW() WHERE id=%s",
                                      (_new_rt, account_id))
                        _rc.commit(); _rc.close()
                        refresh_token = _new_rt
                        print(f"  [inbox] new refresh_token saved id={account_id}")
                    except Exception as _rte:
                        print(f"  [inbox] refresh_token save failed: {_rte}")
            ...
            if "400" in err_s or "401" in err_s:
                if not hasattr(poll_inbox_for_verify, "_retried_400"):
                    poll_inbox_for_verify._retried_400 = True
                    log("  [inbox] 400/401 首次，清除token重试一次...")
                    access_token = ""
                    time.sleep(interval_sec)
                    continue
                log("  [inbox] token已废弃(400/401)，两次失败放弃")
                poll_inbox_for_verify._retried_400 = False
        ...
        async with Chrome(options=opt, connection_port=_free_port()) as browser:
            ...
            if ref_code:
                ...
                if not _ref_cookie_ok:
                    log("  [FIX-REF-COOKIE] ref-code cookie still missing; injecting JS cookie")
                    try:
                        await tab.execute_script(
                            f"document.cookie = 'ref-code={ref_code}; domain=unitool.ai; path=/; SameSite=Lax';",
                            return_by_value=True)
                        log(f"  [FIX-REF-COOKIE] injected: {ref_code}")
                    except Exception as _inj_e:
                        log(f"  [FIX-REF-COOKIE] inject warn: {_inj_e}")
...
async def process_one_account(acct_id, email, password, refresh_token, ref_code=""):
    ...
    log("[step2] 轮询inbox...")
    verify_url = poll_inbox_for_verify(refresh_token, timeout_sec=300, interval_sec=20, account_id=acct_id)
    
    if not verify_url or verify_url == "__TOKEN_INVALID__":
        if verify_url == "__TOKEN_INVALID__":
            log("[step2] Graph token 400 (refresh_token失效) -> verify_pending")
        else:
            log("[step2] 未找到verify链接")
        mark_account(acct_id, "ms_token_expired", "graph_400")
...
async def retry_pending_account(acct_id, email, password, refresh_token):
    ...
    if not verify_url or verify_url == "__TOKEN_INVALID__":
        if verify_url == "__TOKEN_INVALID__":
            log("[retry-step2] Graph token 400 -> verify_pending")
        else:
            log("[retry-step2] 仍未找到verify链接，重标pending")
        mark_account(acct_id, "ms_token_expired", "graph_400_retry")
        return False
